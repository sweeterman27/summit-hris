...
import { Users, Clock, CalendarDays, TrendingUp, AlertCircle, Calendar, Flag, ChevronRight, MapPin, FileText } from 'lucide-react';
        ...
        <div className="flex items-end justify-between">
          <div>
            <h1 className="text-4xl font-black text-white tracking-tighter uppercase">
              {isAdmin ? 'Enterprise Intelligence' : 'Personal Intelligence'}
            </h1>
            <div className="flex items-center gap-2 mt-1">
              <div className="h-px w-8 bg-brand-gold/40" />
              <p className="text-brand-gold/60 text-[10px] font-black uppercase tracking-[0.4em]">
                {isAdmin ? 'Real-time operational overview' : 'Your operational pulse and proficiency'}
              </p>
            </div>
          </div>
        </div>
            ...
            <div className="bg-white/[0.02] border border-white/5 rounded-[2.5rem] p-8 backdrop-blur-xl relative overflow-hidden group hover:border-brand-gold/10 transition-all shadow-deep">
               <div className="flex items-center justify-between mb-8 relative z-10">
                  <div className="flex items-center gap-3">
                    ...
                    <span className="text-[10px] font-black uppercase tracking-[0.3em] text-white/30">Upcoming Operations</span>
                  </div>
               ...
               <div className="space-y-4 relative z-10">
                  {isEventsLoading ? (
                    ...
                    <div className="py-12 flex flex-col items-center justify-center border border-dashed border-white/5 rounded-[2rem]">
                       ...
                       <p className="text-[9px] font-black uppercase tracking-widest text-white/10 italic">No scheduled ops</p>
                    </div>
                  ) : (
                ...
                <>
                  {isAdmin && (
                    <StatsCard 
                      ...
                      label="Personnel active in registry"
                      icon={Users}
                    />
                  )}
                  <StatsCard 
                    ...
                    label="Active shift connection"
                    icon={Clock}
                    color={stats?.personal?.isClockedIn ? 'text-emerald-400' : 'text-brand-gold'}
                  />
                  {isAdmin && (
                    <StatsCard 
                      title="Leave Logistics"
                      value={stats?.global?.pendingLeave}
                      label="Operational absence requests"
                      icon={CalendarDays}
                    />
                  )}
                  <StatsCard 
                    title="Core Proficiency"
                    value={`${stats?.personal?.okrProgress || 0}%`}
                    label={`Proficiency: ${stats?.personal?.summitLevel || 'Initializing...'}`}
                    icon={TrendingUp}
                  />
                </>
              )}
            ...
            <div className="bg-[#0f0d04] border border-brand-gold/10 rounded-[3rem] p-12 relative overflow-hidden group shadow-deep">
               <div className="relative z-10">
                 <div className="flex items-center gap-4 mb-10">
                    ...
                    <div>
                      <span className="text-[10px] font-black uppercase tracking-[0.5em] text-brand-gold">Operations Terminal</span>
                      <p className="text-white/20 text-[9px] font-black uppercase tracking-widest mt-1">Authorized Deployment Access</p>
                    </div>
                 </div>
                 <div className={`grid ${isAdmin ? 'grid-cols-2 lg:grid-cols-4' : 'grid-cols-3'} gap-8`}>
                    ...
                    <a href="/calendar" className="bg-white/[0.02] hover:bg-brand-gold/[0.08] border border-white/5 hover:border-brand-gold/20 rounded-3xl p-8 transition-all duration-500 group/btn shadow-xl hover:-translate-y-1">
                       ...
                       <h4 className="text-white font-black text-sm mb-1 uppercase tracking-tight">Calendar</h4>
                       <p className="text-[10px] text-white/20 font-bold uppercase tracking-widest">Enterprise Ops</p>
                    </a>
                    {isAdmin && (
                      <a href="/dashboard/accomplishments" className="bg-white/[0.02] hover:bg-brand-gold/[0.08] border border-white/5 hover:border-brand-gold/20 rounded-3xl p-8 transition-all duration-500 group/btn shadow-xl hover:-translate-y-1">
                        <FileText className="text-brand-gold/40 group-hover/btn:text-brand-gold mb-5 transition-all duration-500 group-hover/btn:scale-110" size={32} strokeWidth={1.5} />
                        <h4 className="text-white font-black text-sm mb-1 uppercase tracking-tight">Audit AR</h4>
                        <p className="text-[10px] text-white/20 font-bold uppercase tracking-widest">Proof-of-Work Registry</p>
                      </a>
                    )}
                    <button 
